...
export async function POST(
  ...
  try {
    ...
    // 2. EXTRACT IMAGES FROM FORM DATA
    ...
    // 4. UPLOAD IMAGES (Vercel Blob or local storage)
    const uploadPromises = images.map(async (image, index) => {
      try {
        ...
        if (useLocalStorage) {
          ...
          // Return local URL (accessible via /uploads/dad-avatars/filename)
          const localUrl = `/uploads/dad-avatars/${filename}`;
          console.log(
            `[Upload Images] ✅ Saved locally ${index + 1}/${images.length}: ${localUrl}`
          );

          return localUrl;
        } else {
          // VERCEL BLOB: Upload to cloud storage
          const blob = await put(`dad-avatars/${filename}`, buffer, {
            access: "public",
          ...
          console.log(
            `[Upload Images] ✅ Uploaded to Vercel Blob ${index + 1}/${images.length}: ${filename}`
          );
        ...
        console.error(
          `[Upload Images] ❌ Failed to upload image ${index + 1}:`,
          error
        );
        throw new Error(
    ...
    console.log(
      `[Upload Images] ✅ Successfully uploaded ${uploadedUrls.length} image(s) ${storageType}`
    );
